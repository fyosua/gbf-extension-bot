...
async function raidSearchRoutine() {
    ...
    if (!currentLoc.includes("quest/assist") && 
        ...
        uiLog(">> [Raid Search] Navigating to Assist Lobby...");
    ...
    while (isRunning && currentExecutionToken === myToken) {
        ...
        if (currentHash.includes("quest/assist")) {
            ...
            && document.querySelector('#popup-body').innerText === "You can only provide backup in up to three raid battles at once.") {
                ...
                uiLog(">> [Raid Search] Cleared '3 Raid Limit' Popup.");
                await sleep(5000);
                continue;
            }
            ...
            && document.querySelector('#popup-body').innerText === "This raid battle has already ended.") {
                await gameClick('.pop-show .btn-usual-ok', true);
                uiLog(">> [Raid Search] Cleared 'Already Ended' Popup.");
                await sleep(1000);
            ...
            && window.getComputedStyle(document.querySelector('.pop-result-assist-raid.pop-show')).display !== 'none') {
                ...
                if (okBtn) {
                    uiLog(">> [Raid Search] ⚠️ Raid already ended! Clearing popup...");
                ...
                if (hasBtn) {
                    ...
                    uiLog(">> [Raid Search] Cleared Popup (Consumed EP or closed alert).");
                    continue;
                }
            }
            ...
            if (searchTab && !searchTab.classList.contains('active')) {
                uiLog(">> [Raid Search] Switching to 'Search' tab...");
            ...
            if (slotBtn && !slotBtn.classList.contains('active')) {
                uiLog(`>> [Raid Search] Selecting Search Slot ${currentSearchSlot}...`);
            ...
            if (noTargetTxt && noTargetTxt.getBoundingClientRect().width > 0) {
                uiLog(">> [Raid Search] No active raids found. Refreshing list...");
            ...
            if (availableRaid) {
                uiLog(">> [Raid Search] Raid found! Joining...");
        ...
        // --- STATE B: Supporter / Summon Selection Page ---
            ...
            if (startBtn && !startBtn.classList.contains('disable')) {
                ...
                const clickedSummon = await selectSummon(">> [Raid Search]");
        ...
        // --- STATE C: Raid Battle ---
            ...
            if(nextBtn && window.getComputedStyle(nextBtn).display !== 'none'){
                uiLog(">> [Raid Search] Done with no loot.");
        ...
        // --- STATE E: The Failsafe ---
            ...
            uiLog(">> [Raid Search] 🔄 Redirecting back to Assist Lobby...");
            window.location.hash = "quest/assist";
            await sleep(2500); 
            continue;
        }
    ...
    if (currentExecutionToken === myToken) {
        ...
        uiLog(">> [Raid Search] 🛑 Engine Stopped Safely.");
    }
}