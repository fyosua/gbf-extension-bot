# 🗡️ Granblue Automation Framework (v1.1.0)
...
* **Robust Error Handling:** Safely catches missing content scripts and connection drops, preventing UI desyncs.
...
* **🗡️ Slime Farmer:** Fully handles supporter selection, battle engagement, result processing, Rank Up popups, and automatic retries. **Features a Dynamic Quest ID input** via the UI to easily adapt to rotating event nodes.
* **⏩ Daily Pro Skip:** Dynamically sweeps through all available Pro Skip raids (Hard+, Omega, Primarch, etc.) using **optimized DOM batch-querying** for maximum browser performance. Automatically halts the engine when all daily skips are complete.
* **🔍 Auto Raid Search:** Autonomously navigates the Assist Lobby, joins targeted raid slots, clears EP popups, selects fallback supporters, and engages the Auto-Attack sequence. Features a configurable wait-time parameter for "Wanpan" (one-punch) leeching, and **automatically refreshes the lobby** if no targets are found.
...
* **Engine Core:** `helpers.js` and `content.js` handle the state machine, API routing, and DOM manipulation.
...
4. Configure any dynamic settings (like Target Quest IDs, Search Slots, or Wait Times) that appear.
